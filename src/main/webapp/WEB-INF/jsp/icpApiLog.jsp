...
<html lang="zh-Hant-TW">
	<head>
	    ...
	    <title>台灣企銀-日誌查詢系統ICPAPILog</title>
	...
	<body class="sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed sidebar-collapseX" style="height: auto;">
		    ...
		    <div class="content-wrapper">
		        <div class="main-content">
		            <div class="main-title">
		                <span class="title-text" id="showtitle">ICPAPILog查詢</span>
		            </div>
		...
		<script>
			...
			var itemsCount = 0;
			var checkClear = 0;
			document.getElementById("queryTable").value = 'IcpApiLog';
			document.getElementById("selectIcpApiLog").innerHTML = '<i class="fa-solid fa-circle nav-icon"></i>';
			document.getElementById("showtitle").innerHTML = document.getElementById("showIcpApiLog").innerHTML + "日誌查詢";
			//查詢資料
			function doquery() {
				let showcount = document.getElementById("showcount").value;
				clicknum = 0;
				$("#jsGrid").jsGrid("reset");
				dojsGrid('queryIcpApiLog', showcount);
			}
				    ...
				    controller:
						{
						    loadData: function(filter) {
						    	if(checkClear != 1){
							        ...
							        return queryajax("/icpApiLog/" + apiurl, requ, "<%=request.getContextPath()%>");
						    	}
						    }
						},
						fields: [
							{ name: "icpId", type: "text", width: 40 , title:"索引" },
							{ name: "icpType", type: "text", width: 140, title:"icpType" },
							{ name: "icpCustId", type: "text", width: 100, title:"icpCustId"},
							...
							{ 
								itemTemplate: function(_, item) {
					                return $("<button>").text("詳細")
					                	.on("click", function() {
					        				document.getElementById("queryId").value = item.icpId;

...
<html>
	<head>
	    ...
	    <title>台灣企銀-日誌查詢系統LpmApiLog</title>
	...
	<body class="sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed sidebar-collapseX" style="height: auto;">
		    ...
		    <div class="content-wrapper">
		        <div class="main-content">
		             <div class="main-title">
						...
						<span class="title-text" id="showtitle">LpmApiLog</span>
		             </div>
		...
		<script>
			...
			var itemsCount = 0;
			var checkClear = 0;
			document.getElementById("queryTable").value = 'LpmApiLog';
			document.getElementById("selectLpmApiLog").innerHTML = '<i class="fa-solid fa-circle nav-icon"></i>';
			document.getElementById("showtitle").innerHTML = document.getElementById("showLpmApiLog").innerHTML + "日誌查詢";
			...
			function doquery() {
				...
				clicknum = 0;
				$("#jsGrid").jsGrid("reset");
				dojsGrid('queryLpmApiLog', showcount);
			}
				    ...
				    controller:
						{
						    loadData: function(filter) {
						    	if(checkClear != 1){
							        ...
							        return queryajax("/lpmApiLog/" + apiurl, requ, "<%=request.getContextPath()%>");
						    	}
						    }
						},
						fields: [
							{ name: "lpmId", type: "text", width: 40 , title:"索引" },
							{ name: "lpmUserId", type: "text", width: 140, title:"身分證字號或統編" },
							{ name: "lpmTxSeq", type: "text", width: 100, title:"交易序號"},
							{ name: "lpmUserCode", type: "text", width: 100, title:"銀行帳號"},
							...
							{ 
								itemTemplate: function(_, item) {
					                return $("<button>").text("詳細")
					                	.on("click", function() {
					        				document.getElementById("queryId").value = item.lpmId;
